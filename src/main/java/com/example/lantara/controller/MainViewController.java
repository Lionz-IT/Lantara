...
public class MainViewController {
    ...
    /** Dipanggil dari DashboardController setiap kali halaman ini ditampilkan */
    public void initData(User user) {
        ...
        // Agar kolom Aksi menyesuaikan role ketika initData dipanggil ulang
    ...
    public void initialize() {
        ...
        // Jenis & Kapasitas diturunkan dari subclass
        ...
        // Kolom Aksi (Edit/Hapus) — hanya untuk manajer
        ...
        // Hindari duplikasi kolom saat initialize dipanggil ulang oleh SceneBuilder/FX
        if (!vehicleTable.getColumns().contains(aksiCol)) {
            vehicleTable.getColumns().add(aksiCol);
        }

        // Data tabel menunjuk ke daftar global
    ...
    protected void handleAddNewVehicle() {
        try {
            ...
            controller.setVehicleList(MainApp.allVehicles); // agar form menambah langsung ke list global
            ...
            // Setelah form ditutup: refresh tampilan + persist ke CSV
            vehicleTable.refresh();
            MainApp.saveAllData();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    /** Aman jika FXML lama masih memanggilnya. Jika tombol Muat Ulang sudah dihapus, method ini tidak pernah dipakai. */
    @FXML
    ...
    public void openEditForm(Vehicle vehicle) {
        try {
            ...
            // Simpan perubahan dan refresh tabel
            MainApp.saveAllData();
            vehicleTable.refresh();
        } catch (IOException e) {
            e.printStackTrace();
